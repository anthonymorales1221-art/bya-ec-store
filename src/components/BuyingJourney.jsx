...
export default function BuyingJourney() {
  ...
  return (
    <section id="como-comprar" className="bg-[var(--ba-warm-white)] px-5 py-24 sm:px-8 sm:py-32 lg:px-10">
      <div className="mx-auto max-w-[1240px]"><div className="max-w-3xl"><p className="ba-kicker">Paso a paso</p><h2 className="ba-section-title mt-4">Comprar es simple</h2><p className="ba-section-copy mt-5">Explora, elige y confirma tu pedido directamente por WhatsApp.</p></div>
        <div className="mt-16 grid gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:gap-24">
          ...
          <div className="divide-y divide-[var(--ba-border)]">{STEPS.map((step, index) => <article key={step.title} ref={(element) => { refs.current[index] = element; }} data-step={index} className="flex min-h-[42vh] flex-col justify-center py-12 lg:min-h-[55vh]"><span className="text-xs font-extrabold uppercase tracking-[0.17em] text-[var(--ba-copper)]">Paso 0{index + 1}</span><h3 className="mt-4 font-display text-4xl font-medium text-[var(--ba-navy-deep)]">{step.title}</h3><p className="mt-4 max-w-xl text-base leading-8 text-[var(--ba-muted)]">{step.text}</p>{index === 0 && <Link to="/tienda" className="ba-arrow-link mt-6 w-fit text-[var(--ba-navy)]">Abrir la tienda <span aria-hidden="true">→</span></Link>}</article>)}</div>
        </div>
      </div>
    </section>
  );
